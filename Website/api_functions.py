...
base_url = "https://gw.api.it.umich.edu/um/aa/ClassroomList"
def get_classroom(public_key, private_key):
    ...
    if response.ok:
        ...
        response_dict = xmltodict.parse(response.text, encoding='utf-8', process_namespaces=False, namespace_separator=':')
...
def get_room_info(room_id, public_key, private_key):
    ...
    if response.ok:
        ...
        response_dict = xmltodict.parse(
            response.text, encoding='utf-8', process_namespaces=False, namespace_separator=':')
...
def get_data_from_endpoint(endpoint, room_id, auth_header, params):
    ...
    endpoint = endpoint.replace("{room_id}", room_id)
